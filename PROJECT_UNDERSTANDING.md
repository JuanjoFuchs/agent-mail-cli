...
6. **Implement specs 002 and 003.** ⏳ Spec 002 has PyPI/GitHub Release verification complete and is waiting on external WinGet approval. Spec 003 has live npm publishing verified for `@juanjofuchs/agent-mail@0.1.4`; npm rejected unscoped `agent-mail` and `agent-mail-cli`, so npm distribution remains scoped. Replace JJ's daily use of `python scripts/mail.py` with the packaged version (`pipx install agent-mail-cli` or `npx -y @juanjofuchs/agent-mail`).
