...
- Spec 003 (npm + `npx`, modeled on ccburn's npm wrapper) is in progress. npm rejected unscoped `agent-mail`; `agent-mail-cli` is the current unscoped package attempt. The installed command remains `agent-mail`; `v0.1.4` is the verification release.
